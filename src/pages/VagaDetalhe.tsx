...
export function VagaDetalhe() {
  ...
  const placasPorPessoa = useMemo(() => {
    const mapa = new Map<string, string[]>();
    for (const v of veiculos) {
      for (const p of v.pessoas) {
        const lista = mapa.get(p.id) ?? [];
        lista.push(placaComHifen(v.placa));
        mapa.set(p.id, lista);
      }
    }
    return mapa;
  ...
  const pessoasFiltradas = useMemo(() => {
    ...
    const comVeiculo = (p: Pessoa) =>
      (placasPorPessoa.get(p.id) ?? []).length > 0;
    const base = (t
      ...
      : pessoasOrdenadas
    ).filter((p) => comVeiculo(p) || dados.pessoaIds.includes(p.id));
    const emOutraVaga = (p: Pessoa) =>
      vinculadas.has(p.id) &&
      !(vaga?.pessoas ?? []).some((vp) => vp.id === p.id);
    const ordem = (p: Pessoa): number => {
      if (dados.pessoaIds.includes(p.id)) return 0;
      if (emOutraVaga(p)) return 2;
      return 1;
    };
    return [...base].sort((a, b) => ordem(a) - ordem(b));
  }, [
    pessoasOrdenadas,
    filtroPessoas,
    dados.pessoaIds,
                  ...
                  <>
                    ...
                    <div className="space-y-1.5 max-h-80 overflow-y-auto border border-pietra-clara rounded-sm p-3">
                      {pessoasFiltradas.map((p) => {
                        ...
                        const placas = placasPorPessoa.get(p.id) ?? [];
                        return (
                          <label
                            key={p.id}
                          ...
                          >
                            ...
                            <span className="flex-1 min-w-0">
                              <span className="text-carbone block truncate">
                                {p.nome}
                              </span>
                              {placas.length > 0 && (
                                <span className="text-xs text-ardesia font-mono block truncate">
                                  {placas.join(", ")}
                                </span>
                              )}
                            </span>
        ...
        <div className="card-corpo space-y-3">
          ...
          <div>
            ...
            {vaga.veiculos.length === 0 ? (
              ...
              <div className="text-carbone text-sm mt-1">
                {vaga.veiculos.map((v, i) => (
                  <span key={v.id}>
                    {i > 0 && ", "}
                    <span className="font-mono font-semibold">
                      {placaComHifen(v.placa)}
                    </span>
                  </span>
                ))}
              </div>
